...
# Create a PGM object with a wide layout to mirror the reference
pgm = daft.PGM(dpi=150, aspect=2.0, observed_style="inner", alternate_style="outer")

# Add professionally styled nodes
pgm.add_node(
    "Lemons",
    "Lemon\n"+r"$X$",
    1,
...
# Add the causal arrow representing the 1747 understanding
pgm.add_edge("Lemons", "Scurvy")

# Render the DAG
fig = pgm.render()
plt.title("Simplified Lind's Model (1747)", fontsize=10)
plt.tight_layout()
...
pgm = daft.PGM(dpi=150, aspect=3.0, observed_style="inner", alternate_style="outer")
...
# Use DAFT's default "inner" node styling (circles), and keep node aspect at 1.0.
# Tweaking y-coordinates gives the diagonal layout seen in the reference.
pgm = daft.PGM(dpi=150, aspect=1.0, alternate_style="inner")
...
pgm.add_node(
    ...
    1.1,
    aspect=1.45,
    # Bigger circles so the label text fits cleanly (matches the reference).
    scale=1.35,
    fontsize=8.5,
)
pgm.add_node(
    "collagen",
    # Single-line label fits better in the circle style.
    "Collagen Synthesis",
    2.0,
    0.5,
    aspect=1.6,
...
pgm.render()
plt.title("Simplified Modern Understanding (1930s onwards)", fontsize=10)
plt.tight_layout()
```
...
### Example: Simplified Modern Understanding DAG

Here's an example (see @fig-example-node) that matches the simplified Vitamin C → Collagen Synthesis → Scurvy diagram:
