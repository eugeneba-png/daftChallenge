...
pgm.add_edge("lemons", "scurvy_prev")

# Render the DAG
fig = pgm.render()
plt.title("1747 Understanding: Lemons Prevent Scurvy", fontsize=10)
plt.tight_layout()
```
...
pgm = daft.PGM(dpi=150, aspect=2.8, observed_style="inner", alternate_style="outer")
...
pgm.render()
plt.title("Misguided Belief (Wrong): Acid → Bacteria Death → Scurvy Prevention", fontsize=10)
plt.tight_layout()
```
...
# 1928: Mechanistic understanding – Vitamin C supports collagen synthesis; loss of collagen leads to scurvy
pgm = daft.PGM(dpi=150, aspect=2.8, observed_style="inner", alternate_style="outer")
...
pgm.render()
plt.title("1928 Understanding: Vitamin C → Collagen Synthesis → Scurvy", fontsize=10)
plt.tight_layout()
```
